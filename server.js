...
app.get("/login", (req, res) => {
  const authUrl = `http://localhost:8000/login/oauth/authorize?client_id=f1321b09fbe6305a0df3&response_type=code&redirect_uri=https://localhost:7443/callback&scope=read&state=${Math.random()
    .toString(36)
    .substring(7)}`;
  ...
  try {
    const tokenResponse = await axios.post(
      `http://localhost:8000/api/login/oauth/access_token`,
      {
        ...
        redirect_uri: "https://localhost:7443/callback",
      },
      {
        headers: {
    ...
    console.log("\nToken response:", tokenResponse.data.access_token);
    ...
    res.redirect("https://localhost:7443");
  } catch (error) {
    console.error(
